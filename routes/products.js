...
router.get('/', function(req, res) {
  ...
  database.table('products as p')    //get product from database
      .join([
        {
          table: "categories as c",
          on: `c.id = p.cat_id`           //check for category ID in product table
        }
      ])
      .withFields(['c.title as category',    //restrict the information displayed
        'p.title as name',
        'p.price',
        'p.quantity',
        'p.description',
      ...
      .slice(startValue, endValue)         //displays products within a certain range
      .sort({id: .1})
      .getAll()
      .then(prods => {
        if (prods.length > 0) {
          ...
          res.json({message: "No products found"});  //error message
        }
      })
